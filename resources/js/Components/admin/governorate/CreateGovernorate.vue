    ...
    <div class="rounded-2xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03]">
      <div class="border-b border-gray-200 px-6 py-4 dark:border-gray-800">
        <h2 class="text-lg font-medium text-gray-800 dark:text-white">
          {{ t('governorates.governorateInformation') }}
        </h2>
      </div>
      ...
      <div class="p-4 sm:p-6 dark:border-gray-800">
        <div class="grid grid-cols-1 gap-5 md:grid-cols-2">
          <div>
            <label for="governorate-name-ar" class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-400">
              {{ t('governorates.nameAr') }}
            </label>
            <input
              ...
              autocomplete="off"
              :placeholder="t('governorates.nameAr')"
          ...
          <div>
            <label for="governorate-name-en" class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-400">
              {{ t('governorates.nameEn') }}
            </label>
            <input
              ...
              autocomplete="off"
              :placeholder="t('governorates.nameEn')"
              class="dark:bg-dark-900 shadow-theme-xs focus:border-brand-300 focus:ring-brand-500/10 dark:focus:border-brand-800 h-11 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 placeholder:text-gray-400 focus:ring-3 focus:outline-hidden dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30"
            />
...
function create() {
  form.post(route('admin.governorates.store'), {
    onSuccess: () => {
      success(t('governorates.governorateCreatedSuccessfully'))
      form.reset()
    },
    onError: () => {
      error(t('governorates.governorateCreationFailed'))
    },
    preserveScroll: true,
  })
}
