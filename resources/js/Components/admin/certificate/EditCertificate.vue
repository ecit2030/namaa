    ...
    <div class="rounded-2xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03]">
      ...
      <div class="p-4 sm:p-6">
        <div class="grid grid-cols-1 gap-5 md:grid-cols-2">
          ...
          <!-- full_name, gender, date_of_birth and address removed -->
        </div>
      </div>
    </div>
      ...
      <div class="p-4 sm:p-6 space-y-6">
        ...
        <div v-if="hasExistingDocument" class="rounded-lg border border-gray-200 bg-white p-4 dark:border-gray-700 dark:bg-gray-900">
          <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div>
              <p class="text-sm font-medium text-gray-700 dark:text-gray-300">{{ t('certificate.currentDocument') }}</p>
        ...
        <div>
          <label class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-400">{{ t('certificate.replaceDocument') }}</label>
          <label for="certificate-document" class="shadow-theme-xs group relative block cursor-pointer rounded-lg border-2 border-dashed border-gray-300 transition hover:border-brand-500 dark:border-gray-800 dark:bg-gray-900 dark:hover:border-brand-500">
            ...
            <div v-else class="flex flex-col gap-4 p-4">
              ...
              <button type="button" @click.stop="removeDocument" class="self-start rounded-full bg-error-500 px-3 py-1 text-xs font-medium text-white shadow-theme-xs">{{ t('certificate.removeDocument') }}</button>
            </div>
...
const initialForm = () => ({
  consultant_id: props.certificate?.consultant_id ?? '',
  // full_name, gender, date_of_birth, address and document_type removed
  document_scan_copy: null,
  status: props.certificate?.status ?? 'pending',
  rejected_reason: props.certificate?.rejected_reason ?? '',
  is_verified: !!props.certificate?.is_verified,
  verified_at: props.certificate?.verified_at ? props.certificate.verified_at.slice(0, 10) : '',
})
...
const hasExistingDocument = computed(() => !!props.certificate?.document_scan_copy)
const existingDocumentName = computed(() => {
  // Use original filename if available, otherwise extract from path
  if (props.certificate?.document_scan_copy_original_name) {
    return props.certificate.document_scan_copy_original_name
  }
  return props.certificate?.document_scan_copy?.split('/').pop() ?? t('certificate.noDocumentUploaded')
})
const existingDocumentViewUrl = computed(() => (hasExistingDocument.value ? route('admin.certificates.document.view', props.certificate.id) : null))
const existingDocumentDownloadUrl = computed(() => (hasExistingDocument.value ? route('admin.certificates.document.download', props.certificate.id) : null))
...
const submit = () => {
  form
    ...
    .post(route('admin.certificates.update', props.certificate.id), {
      onSuccess: () => {
        success(t('certificate.certificateUpdatedSuccessfully'))
        removeDocument()
        form.transform((data) => data)
      },
      onError: () => {
        error(t('certificate.certificateUpdateFailed'))
        form.transform((data) => data)
      },
      preserveScroll: true,
      forceFormData: true,
