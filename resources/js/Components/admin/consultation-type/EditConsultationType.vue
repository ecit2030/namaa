<template>
  <div class="space-y-6">
          ...
          <div class="grid grid-cols-1 gap-5 md:grid-cols-2">
            <div>
              ...
              <input
                ...
                :placeholder="t('consultationTypes.name')"
              />
    ...
    <div class="rounded-2xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03]">
      <div class="border-b border-gray-200 px-6 py-4 dark:border-gray-800">
        <h2 class="text-lg font-medium text-gray-800 dark:text-white">{{ t('consultationTypes.iconManagement') }}</h2>
      </div>
      <div class="p-4 sm:p-6">
        <div v-if="currentIconUrl" class="mb-6">
          ...
          <div class="flex items-center gap-4">
            <div class="flex h-16 w-16 items-center justify-center rounded-lg border border-gray-200 bg-gray-50 dark:border-gray-700 dark:bg-gray-800">
              <img :src="currentIconUrl" :alt="props.consultation_type.name" class="h-12 w-12 object-contain" />
            </div>
            <button
              type="button"
              @click="removeIcon"
        ...
        <div>
          <label class="mb-2 block text-sm font-medium text-gray-700 dark:text-gray-400">
            {{ currentIconUrl ? t('consultationTypes.changeIcon') : t('consultationTypes.uploadIcon') }}
          </label>
          <div class="flex items-center gap-4">
            <input
              ref="iconInput"
...
const form = useForm({
  _method: 'PUT',
  name: props.consultation_type.name,
  slug: props.consultation_type.slug,
  is_active: props.consultation_type.is_active,
  icon: null,
  remove_icon: false,
})
...
const currentIconUrl = computed(() => {
  ...
  return iconPreviewUrl.value || props.consultation_type.icon_url
})
...
function update() {
  form.post(route('admin.consultation-types.update', props.consultation_type.id), {
    onSuccess: () => {
      success(t('consultationTypes.updated'))
      selectedIcon.value = null
      if (iconPreviewUrl.value) {
        URL.revokeObjectURL(iconPreviewUrl.value)
        iconPreviewUrl.value = null
    ...
    onError: () => {
      error(t('consultationTypes.updateFailed'))
    },
    preserveScroll: true,
  })
}
