            ...
            <table class="w-full min-w-full">
                    ...
                    <tr>
                        ...
                        <th class="px-4 py-3 text-start border border-gray-100 dark:border-gray-800">
                            <div
                                ...
                                @click="sortBy('name')"
                            >
                                <p class="font-medium text-gray-700 text-theme-xs dark:text-gray-400">{{ t('consultationTypes.name') }}</p>
                        ...
                        <th class="px-4 py-3 text-start border border-gray-100 dark:border-gray-800">
                            <div
                                ...
                                @click="sortBy('slug')"
                            >
                                <p class="font-medium text-gray-700 text-theme-xs dark:text-gray-400">{{ t('consultationTypes.slug') }}</p>
                                <span class="flex flex-col gap-0.5">
                                    <svg class="fill-gray-300 dark:fill-gray-700" width="8" height="5" viewBox="0 0 8 5" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M4.40962 0.585167C4.21057 0.300808 3.78943 0.300807 3.59038 0.585166L1.05071 4.21327C0.81874 4.54466 1.05582 5 1.46033 5H6.53967C6.94418 5 7.18126 4.54466 6.94929 4.21327L4.40962 0.585167Z" fill=""/></svg>
                                    <svg class="fill-gray-300 dark:fill-gray-700" width="8" height="5" viewBox="0 0 8 5" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M4.40962 4.41483C4.21057 4.69919 3.78943 4.69919 3.59038 4.41483L1.05071 0.786732C0.81874 0.455343 1.05582 0 1.46033 0H6.53967C6.94418 0 7.18126 0.455342 6.94929 0.786731L4.40962 4.41483Z" fill=""/></svg>
                                </span>
                ...
                <tbody>
                    <tr
                        ...
                        :key="item.id"
                        :class="{ 'bg-gray-50 dark:bg-gray-900': item.selected }"
                    >
                            ...
                            >
                                <span class="relative">
                                    ...
                                    <span
                                        ...
                                        class="flex h-4 w-4 items-center justify-center rounded-sm border-[1.25px]"
                                    >
                                        <span :class="item.selected ? '' : 'opacity-0'">
                                            <svg
                                                width="12"
                                                height="12"
                                                viewBox="0 0 12 12"
                        ...
                        <td class="px-4 py-3 border border-gray-100 dark:border-gray-800">
                            <p class="text-gray-700 text-theme-sm dark:text-gray-400">{{ item.slug }}</p>
                        </td>
                        <td class="px-4 py-3 border border-gray-100 dark:border-gray-800">
                            <span
                                class="inline-flex items-center justify-center gap-1 rounded-full px-2.5 py-0.5 text-xs font-medium"
                                :class="{
                                    'bg-green-50 text-green-600 dark:bg-green-500/15 dark:text-green-500': item.is_active,
                                    'bg-error-50 text-error-600 dark:bg-error-500/15 dark:text-error-500': !item.is_active,
                                }"
                            >
                        ...
                        <td class="px-4 py-3 border border-gray-100 dark:border-gray-800">
                            <label :for="'toggle-' + item.id" class="cursor-pointer">
                                <div class="relative">
                                    <input
                                        type="checkbox"
                                        :id="'toggle-' + item.id"
                                    ...
                                    <div
                                        ...
                                        :class="item.is_active ? 'bg-brand-500 dark:bg-brand-500' : 'bg-gray-200 dark:bg-white/10'"
                                    ></div>
                                    <div
                                        :class="[item.is_active ? 'rtl:translate-x-[-100%] ltr:translate-x-full' : 'translate-x-0']"
...
const canCreate = computed(() => hasAnyPermission(['consultation-types.create', 'consultation-types.store', 'consultation-types.add']))
const canView = computed(() => hasAnyPermission(['consultation-types.view', 'consultation-types.show', 'consultation-types.read']))
const canEdit = computed(() => hasAnyPermission(['consultation-types.update', 'consultation-types.edit']))
const canDelete = computed(() => hasAnyPermission(['consultation-types.delete', 'consultation-types.destroy']))

const { t } = useI18n()
const { success, error } = useNotifications()

const props = defineProps({ consultation_types: Object })

const search = ref('')
const sortColumn = ref('name')
const sortDirection = ref('asc')
const currentPage = ref(props.consultation_types?.current_page ?? 1)
const perPage = ref(props.consultation_types?.per_page ?? 10)
const selectAll = ref(false)

function goToCreate() {
    router.visit(route('admin.consultation-types.create'))
}

function goToView(id) {
    router.visit(route('admin.consultation-types.show', id))
}

function goToEdit(id) {
    router.visit(route('admin.consultation-types.edit', id))
}
...
function confirmDelete() {
    if (itemToDeleteId.value) {
        router.delete(route('admin.consultation-types.destroy', itemToDeleteId.value), {
            onSuccess: () => {
                success(t('consultationTypes.deleted'))
                closeDeleteModal()
            },
            onError: () => {
                error(t('consultationTypes.deleteFailed'))
                closeDeleteModal()
            },
            preserveScroll: true,
        })
...
function toggleItemStatus(item) {
    ...
    const url = wasActive ? route('admin.consultation-types.deactivate', { id: item.id }) : route('admin.consultation-types.activate', { id: item.id })
    router.patch(url, {}, {
        preserveState: true,
        preserveScroll: true,
        onError: () => {
...
const filteredData = computed(() => {
    ...
    return (props.consultation_types?.data || [])
        .filter((item) => {
...
const totalEntries = computed(() => props.consultation_types?.total || filteredData.value.length)
const startEntry = computed(() => props.consultation_types?.from || 1)
const endEntry = computed(() => props.consultation_types?.to || filteredData.value.length)
const totalPages = computed(() => props.consultation_types?.last_page || 1)
...
watch(
    () => props.consultation_types?.current_page,
    (val) => {
...
watch(
    () => props.consultation_types?.per_page,
    (val) => {
