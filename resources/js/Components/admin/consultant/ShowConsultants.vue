<template>
  <div class="overflow-hidden">
    ...
    <div v-if="paginatedData.length" class="grid grid-cols-1 gap-4 px-4 py-4 border border-t-0 border-gray-200 dark:border-gray-800 sm:grid-cols-2 xl:grid-cols-3">
      <article
        v-for="consultant in paginatedData"
        :key="consultant.id"
      ...
      >
        ...
        <div class="flex items-center justify-between border-t border-gray-200 px-5 py-4 dark:border-gray-800">
          <div>
            <p class="text-xs font-medium uppercase tracking-wide text-gray-400">{{ t('common.rating') }}</p>
            <p class="mt-1 text-base font-semibold text-gray-800 dark:text-white/90">
              {{ (consultant.rating_avg ?? 0).toFixed?.(2) ?? consultant.rating_avg }}
              <span class="text-xs text-gray-500 dark:text-gray-500">({{ consultant.ratings_count ?? 0 }})</span>
            </p>
          </div>
          <span
            class="inline-flex items-center justify-center gap-1 rounded-full px-3 py-1 text-xs font-medium"
            :class="consultant.is_active ? 'bg-green-50 text-green-600 dark:bg-green-500/15 dark:text-green-400' : 'bg-error-50 text-error-600 dark:bg-error-500/15 dark:text-error-400'"
          >
        ...
        <div class="flex items-center justify-between border-t border-gray-200 px-5 py-4 dark:border-gray-800">
          <div class="flex items-center gap-2">
            <Tooltip :text="t('messages.notAuthorized')" :show="!canView">
              <button
                :disabled="!canView"
                @click="handleViewClick(consultant.id)"
            ...
            <Tooltip :text="t('messages.notAuthorized')" :show="!canDelete">
              <button
                :disabled="!canDelete"
                @click="handleDeleteClick(consultant.id)"
          ...
          <!-- Toggle Switch -->
          <label :for="'toggle-' + consultant.id" class="cursor-pointer">
            <div class="relative">
              <input
                type="checkbox"
                :id="'toggle-' + consultant.id"
                class="sr-only"
                :checked="consultant.is_active"
                @change="toggleConsultantStatus(consultant)"
              />
              <div
                class="block h-6 w-11 rounded-full"
                :class="consultant.is_active ? 'bg-brand-500 dark:bg-brand-500' : 'bg-gray-200 dark:bg-white/10'"
              ></div>
              <div
                :class="consultant.is_active ? 'translate-x-full rtl:-translate-x-full' : 'translate-x-0'"
    ...
    <div v-else class="border border-t-0 rounded-b-xl border-gray-200 px-4 py-12 text-center text-sm text-gray-500 dark:border-gray-800 dark:text-gray-400">
      {{ t('consultants.noConsultantsFound') }}
    </div>
  ...
  <DangerAlert
    ...
    :message="t('messages.deleteConsultantConfirmation')"
...
const canCreate = computed(() => hasAnyPermission(['consultants.create', 'consultants.store', 'consultants.add']))
const canView = computed(() => hasAnyPermission(['consultants.view', 'consultants.show', 'consultants.read']))
const canEdit = computed(() => hasAnyPermission(['consultants.update', 'consultants.edit']))
const canDelete = computed(() => hasAnyPermission(['consultants.delete', 'consultants.destroy']))
...
function goToEdit(id) {
  router.visit(route('admin.consultants.edit', id))
}
...
const consultantToDeleteId = ref(null)

function openDeleteModal(id) {
  consultantToDeleteId.value = id
...
function closeDeleteModal() {
  ...
  consultantToDeleteId.value = null
}
function confirmDelete() {
  if (!consultantToDeleteId.value) return
  router.delete(route('admin.consultants.destroy', consultantToDeleteId.value), {
    onSuccess: () => {
      success(t('consultants.consultantDeletedSuccessfully'))
      closeDeleteModal()
    },
    onError: () => {
      error(t('consultants.consultantDeletionFailed'))
      closeDeleteModal()
    },
    preserveScroll: true,
  })
...
function toggleConsultantStatus(consultant) {
  const wasActive = consultant.is_active
  consultant.is_active = !wasActive

  const url = wasActive
    ? route('admin.consultants.deactivate', { id: consultant.id })
    : route('admin.consultants.activate', { id: consultant.id })
...
const totalEntries = computed(() => props.consultants?.total || filteredData.value.length)
const startEntry = computed(() => props.consultants?.from || 1)
const endEntry = computed(() => props.consultants?.to || filteredData.value.length)
const totalPages = computed(() => props.consultants?.last_page || 1)
...
watch(
  () => props.consultants?.current_page,
  (val) => {
